...
options=("/boot" "Static files of the boot loader" off    # any option can be set to default to "on"
	"/home" "User home directories" off
...
rootpasswd=$(dialog --backtitle "Archlinux Installation" --title "Root password" --passwordbox "Please, enter the root password" 8 36 2>&1 > /dev/tty)
...
username=$(dialog --backtitle "Archlinux Installation" --title "User creation" \
					--form "Please, enter the user configuration" 0 0 0 \
						"Username :" 1 1 "boban" 1 12 25 30 \
						"Real name:" 2 1 "Boban" 2 12 25 30 2>&1 > /dev/tty)
...
#Prepare the system to boot directly to root and run the post-install script
